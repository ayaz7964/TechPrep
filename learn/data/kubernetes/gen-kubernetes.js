...
function svgBox(x, y, w, h, fill, label) {
  return '<rect x="'+x+'" y="'+y+'" width="'+w+'" height="'+h+'" rx="4" fill="'+fill+'" stroke="#ccc" stroke-width="1"/>' +
    '<text x="'+(x+w/2)+'" y="'+(y+h/2+4)+'" text-anchor="middle" font-size="11" fill="#555">'+label+'</text>';
}
for (var i = 0; i < topics.length; i++) {
  ...
  if (!t.diagramSvg) {
    t.diagramSvg = svgW(500, 160, t.title, svgBox(20, 40, 460, 90, '#e8f4f8', t.title + ' concept diagram'));
  }
}
