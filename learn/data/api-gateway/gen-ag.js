...
]),'Declarative Kong config for DB-less mode â€” ideal for GitOps.'),
...
'Ambassador uses declarative config through Kubernetes CRDs â€” no separate config files needed.',
...
q('What proxy does Ambassador use?','Envoy Proxy â€” high-performance C++ proxy from Lyft.'),
...
'Traefik auto-discovers from Docker labels, K8s Ingress/CRDs, Consul, etcd â€” no manual config needed.',
...
[
d('Auto Service Discovery','Traefik watches provider APIs: K8s (Ingress/CRD/Service), Docker (container labels), Consul/etcd (KV stores). New service deployed = Traefik auto-creates routers and updates LB â€” no config reload needed.'),
...
'AWS API Gateway is like a fully automated airport terminal managed by AWS. You tell them gates (endpoints), who can board (auth), and passenger limits (rate limiting). They handle building, security, maintenance. You focus on flights (backend). Everything scales automatically â€” pay per passenger.',
[
...
q('Can API Gateway integrate with VPC?','Yes, via VPC Link â€” private integration with NLB/ALB in a VPC.'),
...
[
d('Token Bucket Algorithm','Bucket holds N tokens, filled at fixed rate (e.g., 10/sec). Each request consumes one token. Empty bucket = 429. Supports bursts: unused tokens accumulate up to capacity. Most popular â€” used by Kong, AWS, Stripe, GitHub. Parameters: capacity (burst) + refill rate.'),
...
'API Gateways centralize authentication â€” implement once instead of duplicating in every microservice.',
...
[
d('API Key Authentication','Simplest: client includes key in header (X-API-Key), query param, or body. Gateway checks against stored keys. Keys are long-lived and static. Good for server-to-server, service accounts, developer portals. Less secure than tokens â€” rotate periodically.'),
d('JWT Bearer Token Auth','Client presents JWT in Authorization: Bearer header. Gateway validates signature (HS256/RS256/ES256), expiration (exp), audience (aud), issuer (iss). Extracts claims (sub, role) into headers. Stateless â€” no DB lookup. JWKS for key rotation.'),
...
d('mTLS (Mutual TLS)','Both client and server present TLS certificates. Gateway validates client cert against trusted CA. Extracts identity from CN/SAN fields. No tokens needed. Strongest auth â€” used in zero-trust, financial APIs, IoT.')
],
...
q('What is mTLS?','Mutual TLS â€” both client and server present certificates for mutual authentication.'),
